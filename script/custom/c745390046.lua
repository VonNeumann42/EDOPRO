...
function s.ssop(e, tp, eg, ep, ev, re, r, rp)
	...
	if c:IsRelateToEffect(e) then
		...
		ei1:SetDescription(3300)
		ei1:SetType(EFFECT_TYPE_SINGLE)
		ei1:SetCode(EFFECT_LEAVE_FIELD_REDIRECT)
		ei1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_CLIENT_HINT)
		ei1:SetReset(RESET_EVENT+RESETS_REDIRECT)
		ei1:SetValue(LOCATION_REMOVED)
		c:RegisterEffect(ei1,true)
	end
end
...
function s.disfilter(c)
	return c:IsXyzSummonable() and c:IsAttribute(ATTRIBUTE_FIRE)
end
