...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--special summon on summon (From Grave)
...
function s.sotg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.sofilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
end
...
function s.soop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.sofilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	if #g>0 then
		tc=g:GetFirst()
