...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--special summon on summon (From Banishment)
...
function s.sotg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.sofilter,tp,LOCATION_REMOVED,0,1,nil,e,tp) end
...
function s.soop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.sofilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp)
	if #g>0 then
		tc=g:GetFirst()
